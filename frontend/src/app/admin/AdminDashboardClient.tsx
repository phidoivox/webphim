...
export default function AdminDashboardClient() {
  ...
  // Map time series chart data directly from Backend API (with fallback if empty)
  const chartData = useMemo<ChartDataPoint[]>(() => {
    if (data?.viewsTimeseries && data.viewsTimeseries.length > 0) {
      const days = chartTimeRange === "7d" ? 7 : chartTimeRange === "14d" ? 14 : data.viewsTimeseries.length;
      const sliced = data.viewsTimeseries.slice(-days);
      return sliced.map((pt) => ({
        label: pt.label,
        value: pt.views,
        secondaryValue: Math.round(pt.views * 0.7),
        date: `Ngày ${pt.label}`,
      }));
    }
    ...
    return points;
  }, [data?.viewsTimeseries, data?.kpis?.totalViews, chartTimeRange]);
            ...
            <div className="mt-3 flex-1 space-y-2.5">
              {loading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className="h-16 rounded-xl bg-surface/40 animate-shimmer border border-white/5"
                  />
                ))
                ...
                <tbody className="divide-y divide-white/[0.05]">
                  {loading ? (
                    Array.from({ length: 5 }).map((_, i) => (
                      <tr key={i} className="animate-shimmer bg-surface/20">
                        <td className="py-3 px-3.5 text-center">
                          <div className="h-3.5 w-3.5 mx-auto rounded bg-white/10" />
                        </td>
                        <td className="py-3 px-3.5">
                          <div className="flex items-center gap-2.5">
                            <div className="h-10 w-8 rounded-lg bg-white/10 shrink-0" />
                            <div className="space-y-1.5 flex-1">
                              <div className="h-3.5 w-36 rounded bg-white/15 animate-pulse" />
                        ...
                        <td className="py-3 px-3.5 text-right">
                          <div className="h-6 w-16 ml-auto rounded-lg bg-white/10" />
                        </td>
                      </tr>
                    ))
                    ...
                    data.topViewedMovies.map((movie, index) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-3.5">
                          <div className="flex items-center gap-2.5">
                            <div className="relative h-10 w-8 shrink-0 overflow-hidden rounded bg-white/5 border border-white/10">
                              {movie.thumbUrl || movie.thumb_url ? (
                                <Image
                                  src={movie.thumbUrl || movie.thumb_url!}
                                  alt={movie.name}
                                  fill
                        ...
                        <td className="py-3 px-3.5 text-right font-medium text-white tabular-nums">
                          {((movie.viewCount ?? movie.view_count) || 0).toLocaleString()}
                        </td>

                        {/* Rating Avg */}
                        <td className="py-3 px-3.5 text-center text-slate-300 tabular-nums">
                          ⭐ {Number(movie.ratingAvg ?? movie.rating_avg ?? 0).toFixed(1)}
                        </td>
